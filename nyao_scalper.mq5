...
input double RiskPercentPerTrade = 0.01;                  // Risk % of Balance (Base Lot Sizing)
input double RiskPips = 50.0;                             // Risk Distance in Pips for % Risk Sizing
...
//+-------------------------------------------------------------------+
//| Pip Size - value of the 2nd-to-last digit of the symbol's price   |
//| (i.e. 10x the broker's Point). E.g. XAUUSD (2 digits, Point=0.01) |
//| -> pip = 0.1; BTCUSD (0 digits, Point=1) -> pip = 10.             |
//+-------------------------------------------------------------------+
double GetPipSize()
{
    double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
...
//+-------------------------------------------------------------------+
//| Pip Value Per 1.0 Lot (in account currency), derived from the     |
//| broker's tick value/size so contract size & currency conversion   |
//| are handled correctly for any symbol.                             |
//+-------------------------------------------------------------------+
double GetPipValuePerLot()
{
    ...
    return (GetPipSize() / tickSize) * tickValue;
}
...
double CalculateDynamicLotSize(double signalScore = 0)
{
   ...
   // Risk-% based sizing: lot = (balance * risk%) / (RiskPips * pip value per lot)
   double riskAmount = balance * (RiskPercentPerTrade / 100.0);
   double pipValuePerLot = GetPipValuePerLot();

   double baseLot = (pipValuePerLot > 0 && RiskPips > 0)
                        ? NormalizeDouble(riskAmount / (RiskPips * pipValuePerLot), 2)
                        : MinLotSize;
   ...
   if(currentLot < BaseLotSize)
    ...
    LogPrint("Dynamic Lot Calculation: Risk=", DoubleToString(RiskPercentPerTrade, 2), "% over ", RiskPips, " pips",
             " | PipValue/Lot=$", DoubleToString(pipValuePerLot, 4),
             " | RiskLot=", DoubleToString(baseLot, 2),
